#!/bin/bash
# Baseline HiSparse (no spec) + DP Attention (dp_size=tp_size=8) server.
# Mirrors run_mtp_hisparse_dp.sh minus the speculative args, for the
# long-context high-concurrency comparison.
cd "$(dirname "$0")"
mkdir -p logs
